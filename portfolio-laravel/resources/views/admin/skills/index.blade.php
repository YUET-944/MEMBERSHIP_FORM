                ...
                <div class="p-6 text-gray-900">
                            ...
                            <table class="min-w-full divide-y divide-gray-300">
                                ...
                                <tbody class="divide-y divide-gray-200 bg-white">
                                    @foreach($skills as $skill)
                                        <tr>
                                            <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{ $skill->name }}</td>
                                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $skill->category ?? '-' }}</td>
                                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                <div class="flex items-center">
                                                    <div class="w-32 bg-gray-200 rounded-full h-2.5">
                                                        <div class="bg-blue-600 h-2.5 rounded-full" style="width: {{ $skill->level }}%"></div>
                                                    </div>
                                                    <span class="ml-2">{{ $skill->level }}%</span>
                                                </div>
                                            </td>
                                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                @if($skill->icon)
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <h3 class="mt-2 text-lg font-medium text-gray-900">No skills</h3>
                            <p class="mt-1 text-gray-500">Get started by adding a new skill.</p>
                            <div class="mt-6">
                                <a href="{{ route('admin.skills.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                    Add Skill
                                </a>
